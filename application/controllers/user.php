...
class User extends CI_Controller
{
    ...
    function manajemen_user($group){
        
        $data_user = $this->userModel->getData('user1');
        $user_table = $this->userModel->get_user_by_group($group);
        $this->load->view('manajemen_user', array(
            'profileData'=>$data_user,
            'userTableData'=>$user_table));
    }
    
    function get_user_data_by_group($group){
        echo json_encode(array('data_user'=>$this->userModel->get_user_by_group($group)));
    }
